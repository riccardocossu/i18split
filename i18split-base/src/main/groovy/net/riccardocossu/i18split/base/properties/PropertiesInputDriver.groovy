...
class PropertiesInputDriver implements InputDriver {
	...
	public Map getFileList() {
		...
		def inputFiles = [:]
		Set<String> keysToIterate = new TreeSet<String>()
		for(f in files) {
			...
			if (matcherProp.find()) {
				...
				if(!masterLocale || fileLocale.equals(masterLocale)) {
					keysToIterate.addAll(p.keySet())
				}
				inputFiles[fileLocale] = p
			}
		}
		masterIterator = keysToIterate.iterator()
